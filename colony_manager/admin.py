from django.contrib import admin
from colony_manager import models
# Register your models here.

admin.site.register(models.Hunger)
admin.site.register(models.Durability)
admin.site.register(models.Radiation)
admin.site.register(models.Disease)
admin.site.register(models.Morale)
admin.site.register(models.MeteorShowers)
admin.site.register(models.Stress)
admin.site.register(models.DemoliorImpact)
admin.site.register(models.Colony)
admin.site.register(models.Planetoid)
